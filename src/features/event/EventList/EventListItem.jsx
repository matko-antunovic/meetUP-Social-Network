...
class EventListItem extends Component {
  render() {
    ...
    <Segment.Group>
        ...
        <Segment>
          <span>
            <Icon name="calendar" /> {format(event.date.toDate(), 'dddd Do MMMM')}   |    <Icon name="clock" /> {format(event.date.toDate(), 'HH:mm')}    h    |         <Icon name="marker" /> {event.city} - {event.venue}
            
          </span>
        </Segment>
        <Segment secondary>
          <List horizontal>
          {event.attendees && objToArr(event.attendees).map((attendee) => (
